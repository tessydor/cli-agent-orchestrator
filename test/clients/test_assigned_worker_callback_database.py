...
def test_pre_reconciliation_schema_migration_adds_columns_and_guard(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(database_file) as conn:
        ...
        conn.execute(
            "CREATE TABLE assigned_worker_callbacks ("
            "assignment_id TEXT PRIMARY KEY, completion_id TEXT NOT NULL, "
            "worker_terminal_id TEXT NOT NULL, caller_id TEXT NOT NULL, "
            "routing_digest TEXT NOT NULL, lifecycle TEXT NOT NULL, "
            "delivery_state TEXT NOT NULL, receiver_state TEXT NOT NULL, "
            "final_result TEXT, final_result_sha256 TEXT, result_reference TEXT, "
            "inbox_message_id INTEGER, attempt_count INTEGER NOT NULL DEFAULT 0, "
            "created_at DATETIME NOT NULL, dispatched_at DATETIME, "
            "completion_observed_at DATETIME, captured_at DATETIME, "
            "first_attempt_at DATETIME, last_attempt_at DATETIME, enqueued_at DATETIME, "
            "acknowledged_at DATETIME, terminal_error_at DATETIME, last_error TEXT)"
        )
        digest = callback_routing_digest(
            "stuck-legacy-assignment", "stuck-legacy-completion", "22222222", "11111111"
        )
        conn.execute(
            "INSERT INTO assigned_worker_callbacks "
            "(assignment_id, completion_id, worker_terminal_id, caller_id, routing_digest, "
            "lifecycle, delivery_state, receiver_state, dispatched_at, created_at, last_error) "
            "VALUES ('stuck-legacy-assignment', 'stuck-legacy-completion', '22222222', "
            "'11111111', ?, 'dispatched', 'retryable', 'unknown', "
            "'2026-01-01 00:00:00', '2026-01-01 00:00:00', "
            "'Authoritative final report is not available yet')",
            (digest,),
        )
        conn.commit()
    monkeypatch.setattr(
    ...
    try:
        stuck = db.get_assigned_worker_callback("22222222")
        ...
        # Never touched provider-truthful state.
        ...
        with db.SessionLocal() as session:
            with pytest.raises(DatabaseError, match="reconciliation evidence is immutable"):
                session.execute(
                    ...
                    .where(
                        db.AssignedWorkerCallbackModel.assignment_id
                        == "stuck-legacy-assignment"
                    )
                    .values(reconciliation_reason="a different, tampered reason")
                )
                session.commit()
            session.rollback()

        untouched = db.get_assigned_worker_callback("22222222")
        assert untouched.reconciliation_reason == "verified via merged PR"
    finally:
        engine.dispose()
...
def test_mark_caller_reconciled_is_idempotent_on_repeat_call(callback_db):
    ...
    assert again.reconciliation_reason == "reason one"
    assert again.reconciliation_evidence == '{"a": 1}'
...
def test_reconciliation_fields_are_all_or_nothing_on_read(callback_db):
    """A partially-written reconciliation (storage corruption) fails closed."""
    _dispatched_stuck_assignment()
    with db.SessionLocal() as session:
        session.connection().exec_driver_sql(
        ...
        session.connection().exec_driver_sql(
            "UPDATE assigned_worker_callbacks SET caller_reconciled_at = '2026-01-01 00:00:00' "
